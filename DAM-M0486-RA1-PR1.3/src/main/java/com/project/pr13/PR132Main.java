...
public class PR132Main {
    ...
    public List<List<String>> llistarCursos() {
        ...
        try {
            ...
            NodeList cursos = (NodeList) xpath.evaluate("//curs", doc, XPathConstants.NODESET);
            ...
            for (int i = 0; i < cursos.getLength(); i++) {
                ...
                int total = alumnes.getLength();

                List<String> fila = List.of(id, tutor, String.valueOf(total));
                cursosInfo.add(fila);
            }

        } catch (Exception e) {
            System.err.println("Error en llistar cursos: " + e.getMessage());
        }
    ...
    public List<List<String>> mostrarModuls(String idCurs) {
        ...
        try {
            Document doc = carregarDocumentXML(xmlFilePath);
            XPath xpath = XPathFactory.newInstance().newXPath();
            String expr = String.format("//curs[@id='%s']/moduls/modul", idCurs);
            NodeList moduls = (NodeList) xpath.evaluate(expr, doc, XPathConstants.NODESET);

            if (moduls.getLength() == 0) {
                System.out.println("No s'ha trobat cap mòdul o curs amb aquest ID.");
                return modulsInfo;
            }
            ...
            for (int i = 0; i < moduls.getLength(); i++) {
                ...
                modulsInfo.add(List.of(idModul, titol));
            }

        } catch (Exception e) {
            System.err.println("Error en mostrar mòduls: " + e.getMessage());
        }
    ...
    try {
        Document doc = carregarDocumentXML(xmlFilePath);
        XPath xpath = XPathFactory.newInstance().newXPath();
        String expr = String.format("//curs[@id='%s']/alumnes/alumne", idCurs);
        NodeList alumnes = (NodeList) xpath.evaluate(expr, doc, XPathConstants.NODESET);

        for (int i = 0; i < alumnes.getLength(); i++) {
            alumnesList.add(alumnes.item(i).getTextContent());
        }

    } catch (Exception e) {
        System.err.println("Error en llistar alumnes: " + e.getMessage());
    }
    ...
    public void afegirAlumne(String idCurs, String nomAlumne) {
        ...
        try {
            Document doc = carregarDocumentXML(xmlFilePath);
            XPath xpath = XPathFactory.newInstance().newXPath();
            String expr = String.format("//curs[@id='%s']/alumnes", idCurs);
            Node alumnesNode = (Node) xpath.evaluate(expr, doc, XPathConstants.NODE);
            ...
            System.out.println("Error en afegir alumne: " + e.getMessage());
        }
    }
        ...
        // *************** CODI PRÀCTICA **********************/
         try {
            ...
            for (int i = 0; i < alumnes.getLength(); i++) {
                ...
                if (alumne.getTextContent().equalsIgnoreCase(nomAlumne)) {
            ...
            System.out.println("Error en eliminar alumne: " + e.getMessage());
        }
    }
