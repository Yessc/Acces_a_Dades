...
public class PR124main {
    ...
    // Mètode per trobar la posició d'un estudiant al fitxer segons el número de
    // registre
    private long trobarPosicioRegistre(RandomAccessFile raf, int registreBuscat) throws IOException {
        // *************** CODI PRÀCTICA **********************/
        raf.seek(0);

        final int RECORD_SIZE = ID_SIZE + NAME_MAX_BYTES + GRADE_SIZE; // calculo tamaño total de 1 registro

        while (raf.getFilePointer() + ID_SIZE <= raf.length()) {
            long posicioInici = raf.getFilePointer();
            int registreActual = raf.readInt();
            if (registreActual == registreBuscat) {
                return posicioInici;
            }
            raf.seek(posicioInici + RECORD_SIZE);// salto al siguiente registro
        }
    ...
    // Mètode que manipula el fitxer i llista tots els estudiants (independent per
    // al test)
    public void llistarEstudiantsFitxer() throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(filePath, "r")) {
            ...
            System.out.println("Lista Estudiants");
    ...
    public void actualitzarNotaFitxer(int registre, float novaNota) throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(filePath, "rw")) {
            ...
            raf.seek(posicio + ID_SIZE + NAME_MAX_BYTES);
    ...
    private String llegirNom(RandomAccessFile raf) throws IOException {
        ...
        return new String(nomBytes, StandardCharsets.UTF_8).trim(); // Substitueix pel teu
    }
    ...
    private void escriureNom(RandomAccessFile raf, String nom) throws IOException {
        ...
        System.arraycopy(nomBytes, 0, buffer, 0, Math.min(nomBytes.length, NAME_MAX_BYTES));
