...
public class PR122main {
    ...
    public static void serialitzarPersones(List<PR122persona> persones) throws IOFitxerExcepcio {
        ...
        fitxer.getParentFile().mkdirs(); //aseguro que la carpeta data existe
         try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(fitxer))) { //ObjectOutput se usa para serializar objetos
        oos.writeObject(persones);
        }catch(IOException e){
            throw new IOFitxerExcepcio("Error en serializar");
        }
